...
The cache is =~/.elfeed/translated/translate-cache.sqlite=.  Its key is the MD5
of the source text and its value is the raw translation.  Writes are committed
per successful batch, so completed work survives later batch failures or an
Emacs crash.
...
The public entry point remains =elfeed-translate.el=.  It loads four internal
...
| =elfeed-translate.el= | Public commands, hooks, collection and batch dispatch |
...
serial and parallel coordination remains in the main facade for now.
