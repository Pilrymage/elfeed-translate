...
| =elfeed-translate-max-concurrent= | =4= | Concurrent request limit (1 = sequential) |
...
| =elfeed-translate-max-consecutive-fatal= | =3= | Circuit-breaker threshold |
...
Since 0.8.0 batches are always dispatched concurrently.  The removed
=elfeed-translate-parallel= toggle is subsumed by
=elfeed-translate-max-concurrent=: set it to 1 for the former strictly
sequential behavior.
...
Retryable failures use exponential backoff with jitter.  The dispatcher holds
the cycle-level busy lock while waiting, preventing a second translation cycle
from starting over the same uncached entries.

** Failure handling

A circuit breaker and a 429 throttle keep scattered transient errors from
aborting an otherwise healthy translation cycle:
...
cycle.  No further queued batches are sent; already in-flight requests
...
| =elfeed-translate-engine.el= | Collection, bounded concurrent dispatch, circuit breaker, 429 throttle and update hooks |
...
dispatch coordination belongs to the engine module.
