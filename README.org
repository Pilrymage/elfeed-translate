...
| =elfeed-translate-output-dir= | =<elfeed-db-directory>/translated= | SQLite and generated RSS directory |
...
The cache is =translate-cache.sqlite= under
=elfeed-translate-output-dir=.  By default that directory is the =translated=
subdirectory of =elfeed-db-directory=; it is not necessarily =~/.elfeed=.  Its
key is the MD5 of the source text and its value is the raw translation.  Writes
are committed per successful batch, so completed work survives later batch
failures or an Emacs crash.
...
| =elfeed-translate-cache.el= | SQLite persistence and cache-key ownership |
...
| =elfeed-translate-engine.el= | Collection, serial/parallel dispatch, fail-fast abort and update hooks |
